...
def preprocess_image(path, shape_r, shape_c):
    original_image = cv2.imread(path)
    padded_image = padding(original_image, shape_r, shape_c, 3)
    ims = cv2.cvtColor(padded_image, cv2.COLOR_BGR2RGB)
    return ims.squeeze()

def preprocess_map(path, shape_r, shape_c):
...
class SALICON_Dataset(Dataset):
    ...
    def __init__(self, cfg, data_root_list, image_set, is_train, transform=None):
        ...
        pattern = '*.jpg'
        if 'MIT1003' in data_root_list:
            if image_set == 'test':
                pattern = '*.jpg'
            else:
                pattern = '*.jpeg'
        self.images = np.array(get_all_file_dir(osp.join(self.data_root_list,'images',image_set), pattern))
        self.images = np.sort(self.images)
        pattern = '*.png'
        if 'MIT1003' in data_root_list or 'cat2000' in data_root_list:
            pattern = '*.jpg'
        self.maps = np.array(get_all_file_dir(osp.join(self.data_root_list,'maps',image_set), pattern))
    ...
    def __getitem__(self,index):
        image = preprocess_image(self.images[index], self.image_size[0], self.image_size[1])
        if len(self.maps) != 0:
            smap = preprocess_map(self.maps[index], self.map_size[0], self.map_size[1])
            if len(self.fixs) > 0:
                fix = preprocess_fix(self.fixs[index], self.map_size[0], self.map_size[1])
        ...
        if self.return_info:
            ...
            info['size'] = cv2.imread(self.images[index]).shape[:2]
